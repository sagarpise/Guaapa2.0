...
class OpenpayController(http.Controller):
    ...
    def get_openpay_tx_data(self, tx_id, cus_id, provider):
        ...
        if acquirer.state == 'enabled':
            if acquirer.openpay_version == 'mxn':
                url = 'https://api.openpay.mx/v1/%s/charges/%s' % (
                    acquirer.openpay_mxn_merchant_id, tx_id)
            else:
                url = 'https://api.openpay.co/v1/%s/charges/%s' % (
                    acquirer.openpay_cop_merchant_id, tx_id)
        else:
            if acquirer.openpay_version == 'mxn':
                url = 'https://sandbox-api.openpay.mx/v1/%s/charges/%s' % (
                    acquirer.openpay_mxn_merchant_id, tx_id)
            else:
                url = 'https://sandbox-api.openpay.co/v1/%s/charges/%s' % (
                    acquirer.openpay_cop_merchant_id, tx_id)
        res = requests.get(url, auth=(acquirer.openpay_mxn_secret_key if acquirer.openpay_version ==
                                      'mxn' else acquirer.openpay_cop_secret_key, ""))
        return res.json()

    @http.route(_return_card_url, type='http', auth='public', csrf=False, save_session=False)
    def openpay_card_form_feedback(self, **post):
        _logger.info(
            'Openpay(Card): got the transaction redirect response as %s', pprint.pformat(post))
        response = self.get_openpay_tx_data(
            post.get('id'), post.get('cus_id'), 'openpay_card')
        _logger.info(
            'Openpay(Card): entering form_feedback with post data %s', pprint.pformat(response))
        request.env['payment.transaction'].sudo(
        ).form_feedback(response, 'openpay_card')
    ...
    def openpay_alipay_form_feedback(self, **post):
        _logger.info(
            'Openpay(Alipay): got the transaction redirect response as %s', pprint.pformat(post))
        response = self.get_openpay_tx_data(
            post.get('id'), post.get('cus_id'), 'openpay_alipay')
        _logger.info(
            'Openpay(Alipay): entering form_feedback with post data %s', pprint.pformat(response))
        request.env['payment.transaction'].sudo(
        ).form_feedback(response, 'openpay_alipay')
        return werkzeug.utils.redirect('/payment/process')
    ...
    def openpay_store_redirect(self, **post):
        _logger.info(
            'Openpay(Store): entering form_feedback with store data %s', pprint.pformat(post))
        request.env['payment.transaction'].sudo(
        ).form_feedback(post, 'openpay_store')
        return werkzeug.utils.redirect('/shop/confirmation')

    @http.route(_webhook_store_url, type='json', auth='public', csrf=False)
    def openpay_webhook_store_feedback(self, **post):
        _logger.info('Openpay(Store): got the transaction webhook response as %s',
                     pprint.pformat(request.jsonrequest))
    ...
    def openpay_bank_redirect(self, **post):
        _logger.info(
            'Openpay(Bank): entering form_feedback with bank data %s', pprint.pformat(post))
        request.env['payment.transaction'].sudo(
        ).form_feedback(post, 'openpay_bank')
        return werkzeug.utils.redirect('/shop/confirmation')

    @http.route(_webhook_bank_url, type='json', auth='public', csrf=False)
    def openpay_webhook_bank_feedback(self, **post):
        _logger.info('Openpay(Bank): got the transaction webhook response as %s',
                     pprint.pformat(request.jsonrequest))
        if request.jsonrequest.get("transaction"):
